module lcd_2_1(rst, clk, LCD_E, LCD_RS, LCD_RW, LCD_DATA, LED_out, number_btn, control_btn, line_btn);
input rst,clk;
...
oneshot_universal #(.WIDTH(13)) O1(clk, rst, {number_btn[9:0],control_btn[1:0],line_btn_t}, {number_btn_t[9:0],control_btn_t[1:0],line_btn_t});
    ...
    if(!rst) begin
        ...
        case(state)
            ...
            DELAY_T : begin
                ...
                line_btn_t_prev <= line_btn_t;
                if (line_btn_t != line_btn_t_prev) begin
                    state <= SET_ADDRESS;
                end
                else state <= |number_btn_t ? WRITE : (|control_btn_t ? CURSOR : DELAY_T);
            end
            WRITE : begin
...
begin
    if(!rst)
        ...
        case(state)
            ...
            SET_ADDRESS : begin
                if (line_btn_t == 1) {LCD_RS, LCD_RW, LCD_DATA} = 10'b0_0_1000_0000;
                else {LCD_RS, LCD_RW, LCD_DATA} = 10'b0_0_1100_0000;
            end
            DELAY_T :
                {LCD_RS, LCD_RW, LCD_DATA} = 10'b0_0_0000_1111;    
            WRITE : begin
